...
export default class Game extends React.Component {
    ...
    roundWinner(name){
        this.setState({roundWinnerName: name, showRoundWinner: true, lockKeboard: true})
    ...
    newRound(note){
        ...
        setTimeout(() => {
            this.setState({ showRoundWinner: false, lockKeboard: false, timer: false })
            play(note)
        }, 3000)
    }
    ...
    render() {
        return (
            <View>
                <StatusBar hidden={true} barStyle="light-content" backgroundColor="rgb(122,77,246)"/>
            ...
            <LinearGradient colors={['#192f6a', '#268cdc', '#94d0ff']} style={{ width: "100%", height: "100%", backgroundColor: "#94d0ff" }}>
                <View style={{ flexDirection: "row", }}>
                    <View style={{ borderColor: "white", borderWidth: 5, top: 70, borderRadius: 150, backgroundColor: "rgb(255, 144, 236)", alignSelf: 'flex-start', alignItems: "center", justifyContent: "center" }}>{this.state.fontLoaded ? <Text style={{ fontFamily: 'kaki', fontSize: 25, alignSelf: 'flex-start', padding: 5 }}>{this.props.navigation.state.params.name}: {this.state.user1Points}</Text> : null}</View>
                    <View style={{ borderColor: "white", borderWidth: 5, marginLeft: "auto", top: 70, borderRadius: 150, backgroundColor: "rgb(154, 255, 153)", alignSelf: 'flex-start', lignItems: "center", justifyContent: "center" }}>{this.state.fontLoaded ? <Text style={{ fontFamily: 'kaki', fontSize: 25, alignSelf: 'flex-start', padding: 5 }}>{this.state.user2Name}:  {this.state.user2Points}</Text> : null}</View>
                </View>
                <View style={{ flexDirection: "column", flex: 2, alignItems: "center", justifyContent: "center" }}>
                    ...
                    { this.state.showRoundWinner? <View style={{position: "absolute" ,borderRadius: 30, borderColor: "white", borderWidth: 5, width: "100%", alignSelf: 'flex-start', zIndex: 3, backgroundColor: "#f6ff64" }}>
                        <Text style={{ paddingTop: 10, fontFamily: 'kaki', fontSize: 25, textAlign: "center" }}>THE WINNER OF THE ROUND IS</Text>
                        <Text style={{ paddingBottom: 10,fontFamily: 'kaki', fontSize: 60, textAlign: "center" }}>{this.state.roundWinnerName}</Text></View>
                        : null}
                ...
                {this.state.lockKeboard ? <View style={{ position: "absolute", height: 350, width: "100%", bottom: 0, opacity: 0.2 }}></View> : null}
            </LinearGradient>
             }
            </View>
